    ...
    this.script.addEventListener('load', () => {
      if (window.hbspt) {
        window.hbspt.forms.create({
          region: 'eu1',
          portalId: '26980278',
          formId: 'e2b15072-580e-4aa4-bc9d-6ee33e752994',
          target: '#hubspotForm',
          onFormReady: function ($form) {
            $form.style.background = '#313135'
            if (this.user.company?.name) {
              $form.getElementsByTagName('input')[0].value = this.user.company.name
            ...
            $form.getElementsByTagName('input')[1].value = this.user.email
            $form.getElementsByTagName('input')[1].readOnly = true
            $form
