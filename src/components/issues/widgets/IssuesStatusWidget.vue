<template>
  <div v-if="loaded">
    <GroupedStatWidget
      ...
      :values="
        user
          ? [summary.opened, summary.in_progress, summary.closed, summary.user]
          : [summary.opened, summary.in_progress, summary.closed]
      "
      :labels="
        user
          ? ['Open', 'In progress', 'Closed', 'Assigned to me']
    ...
    >
      <template #chart1>
        <div
          class="is-clickable"
          @click="$router.push(getIssueIndexURL(project, { status: 'opened' }))"
        >
          <ApexChart
            :height="opened.chart.height"
            :type="opened.chart.type"
      ...
      <template #chart2>
        <div
          class="is-clickable"
          @click="$router.push(getIssueIndexURL(project, { status: 'in_progress' }))"
        >
          <ApexChart
            :height="in_progress.chart.height"
            :type="in_progress.chart.type"
            :series="in_progress.series"
            :options="in_progress"
          />
        </div>
      </template>
      <template #chart3>
        <div
          class="is-clickable"
          @click="$router.push(getIssueIndexURL(project, { status: 'closed' }))"
        >
          <ApexChart
            :height="closed.chart.height"
            :type="closed.chart.type"
            :series="closed.series"
            :options="closed"
          />
        </div>
      </template>
      <template v-if="user" #chart4>
        <div
          class="is-clickable"
          @click="$router.push(getIssueIndexURL(project, { user_id: user.id }))"
        >
          <ApexChart
            :height="userStats.chart.height"
            :type="userStats.chart.type"
