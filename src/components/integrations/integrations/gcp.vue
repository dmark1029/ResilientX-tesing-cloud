    ...
    <form @submit.prevent="save" @keydown="form.onKeydown($event)">
      ...
      <div class="columns is-multiline">
        ...
        <div class="column is-12">
          <div class="field">
            <textarea
              v-model="form.application_credentials"
              type="text"
              :disabled="!canPerformAction('integrations', 'create')"
              class="textarea"
              placeholder="Enter application credentials"
            />
            <HasError :form="form" field="application_credentials" />
          </div>
        </div>
      </div>
...
export default {
  ...
  methods: {
    ...
    getIntegrations() {
      return axios
        .get(`/integration/gcp/${this.project.id}`)
        .then((res) => {
    ...
    save() {
      return this.form
        .post(`/integration/gcp/${this.project.id}`)
        .then(() => {
          this.form.reset()
          this.getIntegrations()
          this.getCollections()
    ...
    async deleteIntegration(item) {
      ...
      if (ok) {
        this.formDelete
          .delete(`/integration/gcp/${this.project.id}/${item.id}`)
          .then(() => {
            this.getIntegrations()
          })
      }
