      ...
      <div class="columns">
        <div class="column is-6">
          <div class="field">
            ...
            />
            <HasError :form="form" field="aws_access_key_id" />
          </div>
        </div>
        <div class="column is-6">
          <div class="field">
            ...
            />
            <HasError :form="form" field="aws_secret_access_key" />
          </div>
        </div>
      </div>
...
export default {
  ...
  methods: {
    ...
    getIntegrations() {
      return axios
        .get(`/integration/aws/${this.project.id}`)
        .then((res) => {
    ...
    save() {
      return this.form
        .post(`/integration/aws/${this.project.id}`)
        .then(() => {
          this.form.reset()
          this.getIntegrations()
          this.getCollections()
    ...
    async deleteIntegration(item) {
      ...
      if (ok) {
        this.formDelete
          .delete(`/integration/aws/${this.project.id}/${item.id}`)
          .then(() => {
            this.getIntegrations()
          })
      }
