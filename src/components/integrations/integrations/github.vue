<template>
  ...
  <div v-else class="pt-2 pb-2">
    <p v-if="hasIntegration" class="mb-3">
      You have already integrated GitHub with this project. If you want to change the
      integration, please update the existing one.
    </p>
    <p v-else class="mb-3">You don't have active GitHub integration with this project.</p>

    <p class="mb-3">Required permissions: Read repository contents</p>

    <form @submit.prevent="save" @keydown="form.onKeydown($event)">
      <div class="field">
        <label class="label">GitHub URL (https://api.github.com)</label>
        <input
...
export default {
  ...
  methods: {
    getIntegrations() {
      return axios
        .get(`/integration/github/${this.project.id}`)
        .then((res) => {
          ...
          if (!this.form.baseurl) {
            this.form.baseurl = 'https://api.github.com'
          }
    ...
    save() {
      return this.form
        .post(`/integration/github/${this.project.id}`)
        .then(() => {
