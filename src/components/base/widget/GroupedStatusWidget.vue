...
const tempLabels = ['Open', 'In Progress', 'Risk Accepted', 'Closed']
const tempValues = [45, 56, 78, 89]
...
const total = computed(() => {
  return tempValues.reduce((acc, value) => acc + value, 0)
})
    ...
    <div class="">
      <div class="widget-body is-flex">
        <div
          ...
          :key="index"
          class="tag chip"
          :class="getClassByLabel(tempLabels[index])"
          :style="{
      ...
      <div class="widget-content is-flex is-justify-content-center">
        ...
        >
          <span
            ...
            :class="getClassByLabel(tempLabels[index])"
          ></span>
          <span class="v-label risk-number">{{ value }}</span>
          <span class="v-label risk-type">{{ tempLabels[index] }}</span>
        </div>
      </div>
    </div>
  </div>
