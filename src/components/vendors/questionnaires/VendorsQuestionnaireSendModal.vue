<template>
  <VModal
    is="div"
    :open="true"
    :title="editable ? `Send questionnaire` : `Send questionnaire`"
    size="big"
      ...
      <div>
        ...
        <div v-else>
          <div class="app-search mb-3">
            <form role="search" @submit.prevent="searchVendors">
              <div class="columns">
              ...
              <div class="datatable-container">
                <table class="table datatable-table is-fullwidth">
                  ...
                  <tbody>
                      ...
                      <tr v-for="itemAsset in items.data" :key="itemAsset.id">
                            ...
                            <VControl>
                              <VSwitchSegment
                                ...
                                @update:modelValue="
                                  (value) => {
                                    if (value) {
                                      selectedVendors.push(itemAsset)
                                    } else {
                                      selectedVendors = selectedVendors.filter(
                        ...
                        <td>
                          <router-link
                            :to="{
                              name: 'VendorView',
                              params: { asset: itemAsset.id },
                            }"
                            >{{ itemAsset.name }}</router-link
                          >
                        </td>
                      </tr>
                    </template>
                    <template v-else>
                      <tr>
                        <td colspan="7" class="has-text-left">No companies found.</td>
                      </tr>
                    </template>
                  </tbody>
                </table>
...
export default {
  ...
  methods: {
    async send() {
        ...
        promises.push(
          axios
            .post(`/questionnaire-link`, {
              questionnaire_id: this.editable.id,
            ...
            .catch(() => {
              hasErrors = true
              this.$store.dispatch('addAlert', {
                text: 'Failed to send questionnaire to ' + vendor.name,
                type: 'danger',
              })
            })
        )
      ...
      if (!hasErrors) {
        this.$store.dispatch('addAlert', {
          text: 'Questionnaire sent to selected companies.',
          type: 'success',
        })
      }
    ...
    async getAssets(page = 1) {
      ...
      return axios
        .get('/vendor', {
          params: {
            q: this.q,
            workspace_id: this.editable.workspace_id,
            contacts: this.hasContacts ? 1 : null,
            has_questionnaire_id: this.hasBeenSent ? 1 : null,
            filled_questionnaire_id: this.editable.id,
            status: 'active',
            page: page,
          },
        })
