<template>
  ...
  >
    ...
    <text
      ...
      text-anchor="middle"
      style="fill: white; font-weight: 700; font-size: 12px"
    >
      {{ truncatedName }}
    </text>

    <AGrade
      v-if="asset.exposure_score >= 0 && asset.exposure_score < 5"
      :x="140"
      :y="28"
    />
    <BGrade
      v-else-if="asset.exposure_score >= 5 && asset.exposure_score < 20"
      :x="140"
      :y="28"
    />
    <CGrade
      v-else-if="asset.exposure_score >= 20 && asset.exposure_score < 40"
      :x="140"
      :y="28"
    />
    <DGrade
      v-else-if="asset.exposure_score >= 40 && asset.exposure_score < 70"
      :x="140"
      :y="28"
    />
    <EGrade
      v-else-if="asset.exposure_score >= 70 && asset.exposure_score < 85"
      :x="140"
      :y="28"
    />
    <FGrade v-else :x="140" :y="28" />

    <text x="74" y="46" style="fill: white; font-size: 12">
      {{ asset.issues_count }} {{ asset.issues_count === 1 ? 'Issue' : 'Issues' }}
    </text>
...
export default {
  ...
  methods: {
    ...
    handleClick() {
      this.$router.push({
        name: 'VRMAssetView',
        params: { asset: this.asset.asset_id },
      })
    },
  },
}
