<template>
  <div class="onboarding-wrapper onboarding-v1">
        ...
        <template #tab="{ activeValue }">
          <div v-if="activeValue === 'team'" class="onboarding-wrap">
            <div class="onboarding-wrap-inner">
              <div class="onboarding-card" @click="showModal('vrm-board')">
                <img class="dark-image" src="/assets/vrm-board.png" alt="" />
                <div class="onboarding-content is-clickable">
              ...
              <div
                ...
                @click="showModal('vrm-company-summary')"
              >
                <img class="dark-image" src="/assets/vrm-company.png" alt="" />
                <div class="onboarding-content is-clickable">
                  <div class="button-wrap">
                    <VButton color="primary" rounded elevated> Generate </VButton>
                  </div>
              ...
              <div class="onboarding-card is-clickable" @click="showModal('vrm-company')">
                <img class="dark-image" src="/assets/vrm-company-technical.png" alt="" />
                <div class="onboarding-content is-clickable">
                  <div class="button-wrap">
                    <VButton color="primary" rounded elevated> Generate </VButton>
                  </div>
    ...
    >
      <template #content>
        <div class="p-5">
          <p v-if="report === 'vrm-board'">
            Generate board report with details of all companies.
          </p>
          <VField
            v-if="report !== 'vrm-board'"
            v-slot="{}"
            class="is-curved-select w-100"
          >
...
export default {
  ...
  data: () => ({
    ...
    isAnyNew: false,
    reports: {
      'vrm-board': 'Board report',
      'vrm-company': 'Company technical report',
      'vrm-company-summary': 'Company report',
    },
  }),
  computed: {
  ...
  methods: {
    getReportText(type) {
      switch (type) {
        case 'vrm-board':
          return 'Board report'
        case 'vrm-company':
          return 'Company technical report'
        case 'vrm-company-summary':
          return 'Company report'
      }
    },
    saveRecurringReport() {
      this.form.platform = 'vrm'
      this.form.workspace_id = this.workspace.id
      if (this.form.type === 'vrm-board') {
        this.form.collection_id = null
      } else {
        if (!this.form.collection_id) {
          this.$store.dispatch('addAlert', {
            text: 'Please select company to generate a report',
            type: 'danger',
          })
          return
        }
    ...
    generate() {
      if (this.report !== 'vrm-board' && !this.collection_id) {
        this.$store.dispatch('addAlert', {
          text: 'Please select company to generate a report',
          type: 'danger',
        })
